...
/*
function basicParameter(){
    // vygenerovanie view vrstvy zakladne parametre
    if (! $(".BasicParameters")[0]){
        $("<div class=BasicParameters></div>").appendTo(".CreateExam");
        $("<label for=name>Name</label>").appendTo(".BasicParameters");
        $("<input type=text name=name>").appendTo(".BasicParameters");
        $("<div class=exams_array></div>").appendTo(".BasicParameters");
        $("<button type=button onclick=selectTemplate() id=Butt_Add_Exam>Pridaj priklad</button>").appendTo(".BasicParameters");
        $("<div class=SecondParameters id=2_param></div>").appendTo(".CreateExam");
        $("<button type=button id=Save_Exams>Uloz ulohu</button>").appendTo(".BasicParameters");
    }else{
        $(".CreateExam").empty();
        $("<div class=BasicParameters></div>").appendTo(".CreateExam");
        $("<label for=name>Name</label>").appendTo(".BasicParameters");
    ...
    if (! $(".CreateTask")[0]){
        ...
        $("#inputJson").val(JSON.stringify(priklad)).focusout(function(){
            ...
            for(pr in priklad){
                if(priklad[pr] !== null){
                    $("#"+pr).empty();
                    $("<img style='width: 59px;height: 59px; left: -1px;top: -1px;' src=../media/img/Number/"+priklad[pr]+".jpg alt=Cislo_"+priklad[pr]+"></a>").appendTo("#"+pr);
                }
            }
