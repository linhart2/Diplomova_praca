...
public class LoggedLevel_1_2 : MonoBehaviour, UnityEngine.EventSystems.IHasChanged
{
    ...
    public void PripniPrikladNaTabulu()
    {
        ...
        {
            var nazov = "Level1 - " + DateTime.Now.ToString("d.M HH:m:s");
            SharedScreen screen = new SharedScreen()
            {
                screeen_locker = false,
                admin_name = _playerData.Name,
    ...
    void generator_uloh()
    {
        ...
        while (_poliaKtoreSaNevykreslia.Count < 1)
        {
        ...
        while (moznosti.Count < 4)
        {
            ...
            if (moznosti.Contains(x) == false && x != int.Parse(_examArray[_poliaKtoreSaNevykreslia[0]]))
            {
                moznosti.Add(x);
            }
        }
