...
public class Level_1_2 : MonoBehaviour, UnityEngine.EventSystems.IHasChanged
{
    ...
    void Start () {
        ...
        HasChanged();
		FirebaseCommunication f = new FirebaseCommunication ();
		var stringsArray = table.Select(i=>i.ToString()).ToArray();
		json = string.Join("", stringsArray);
		Debug.Log (json);
		f.UpdateResult (json);
    }
    ...
    IEnumerator congrats_hide()
    {
        ...
        gratulation.enabled = false;
        Application.LoadLevel(Random.RandomRange(1, 3));
    }
